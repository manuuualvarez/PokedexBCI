...
final class PokemonDetailViewModelTests: XCTestCase {
    ...
    func testStatProperties() {
        ...
        // Test fallback stats when no stats are provided
        let emptyStats = emptyViewModel.stats
        XCTAssertEqual(emptyStats.count, 5, "Should have 5 fallback stats")
        XCTAssertEqual(emptyStats[0].name, "Height")
        XCTAssertGreaterThan(emptyStats[0].value, 0, "Fallback stats should be positive")
    }
    ...
    func testEdgeCaseHandling() {
        ...
        // Stats should use fallbacks
        XCTAssertFalse(invalidViewModel.stats.isEmpty, "Should provide fallback stats")
    }
} 
